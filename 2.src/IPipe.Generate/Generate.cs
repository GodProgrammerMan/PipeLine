...
namespace IPipe.Generate
{
    public class Generate
    {
        public static void Main(string[] args)
        {
            ...
            var modelResult = FrameSeed.CreateModels(myContext);
            //生成单个iRepository的CreateIRepositorys(myContext,"表名")
            var iRepositoryResult = FrameSeed.CreateIRepositorys(myContext);
            //生成单个iServices的iServicesResult(myContext,"表名")
            var iServicesResult = FrameSeed.CreateIServices(myContext);
            //生成单个repository的iServicesResult(myContext,"表名")
            var repositoryResult = FrameSeed.CreateRepository(myContext);
            //生成单个services的CreateServices(myContext,"表名")
            var servicesResult =  FrameSeed.CreateServices(myContext);
