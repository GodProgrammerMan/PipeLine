...
namespace IPipe.Common.DB
{
    public class BaseDBConfig
    {
        ...
        public static (List<MutiDBOperate>, List<MutiDBOperate>) MutiInitConn()
        {
            ...
            bool CQRSEnabled = false;
            bool MutiDBEnabled = false;
            string MainDB;
            ...
            if (listdatabase.Count == 0) {
                using (StreamReader file = File.OpenText($"{AppDomain.CurrentDomain.BaseDirectory}//appsettings.json"))
                {
                    using (JsonTextReader reader = new JsonTextReader(file))
                    {
                        ...
                        var value = o["DBS"].ToString();
                        CQRSEnabled = bool.Parse(o["CQRSEnabled"].ToString());
                        MutiDBEnabled = bool.Parse(o["MutiDBEnabled"].ToString());
                        MainDB = o["MainDB"].ToString();
                        listdatabase = JsonHelper.ParseFormByJson<List<MutiDBOperate>>(value);
                    }
                }
                foreach (var item in listdatabase)
                {
                    ...
                    {
                        case 0:
                            item.DbType = DataBaseType.MySql;
                            break;
                        case 1:
                            item.DbType = DataBaseType.SqlServer;
                            break;
                        case 2:
                            item.DbType = DataBaseType.Sqlite;
                            break;
                        case 3:
                            item.DbType = DataBaseType.Oracle;
                            break;
                        case 4:
                            item.DbType = DataBaseType.PostgreSQL;
                            break;
                    }
                }
                // 单库，且不开启读写分离，只保留一个
                if (!CQRSEnabled && !MutiDBEnabled)
                {
                    ...
                    {
                        var dbFirst = listdatabase.FirstOrDefault(d => d.ConnId == MainDB);
                        if (dbFirst == null)
                        {
