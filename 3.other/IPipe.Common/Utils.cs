...
namespace IPipe.Common
{
    public static class Utils
    {
        ...
        public static Dictionary<string, string> ActionType()
        {
            ...
            dic.Add("Replace", "替换");
            dic.Add("Print", "打印");
            dic.Add("AllPhone", "手机全号");
            dic.Add("Distribution", "分配");
        ...
        /// 是否是手机号码
        /// </summary>
        /// <param name="sAvailability">需要判断的字符串</param>
        /// <returns>是/否</returns>
        public static bool IsMobilePhone(string sAvailability)
        {
            if (string.IsNullOrEmpty(sAvailability))
                return false;
            else if (sAvailability.Trim().Length != 11 || !Regex.IsMatch(sAvailability.Trim(), "^1[3456789]\\d{9}$"))
                return false;
            else
                return true;
        }
        ...
        public static string CompanyDate(DateTime dateStr1, DateTime dateStr2)
        {
            var msg = ">";
        ...
        #region base64转图片
        public static Image Base64ToImg(string strBase64)
        {
            //去掉头部
...

            {
                ...
                strBase64 = Regex.Match(strBase64, "(?<=;base64,).*").Value;
        ...
        #region 上传图片，并打水印
        /// <summary>
        /// 返回路径 -上传图片（base64），并打水印
        /// </summary>
        /// <returns></returns>
        public static string UploadImage(string strBase64, string path, string fileName, string text, int paddingTop, int paddingLeft
            , Color textColor,bool isWaterMark)
        {
            string imgurl = "http://image.ipipe.com";
            try
            {
                AddWatermarkAndSave(path, fileName, text, Base64ToImg(strBase64), paddingTop, paddingLeft, textColor, isWaterMark, out imgurl);
            }
            catch (Exception e){
            }
            return imgurl;
        ...
        /// <returns></returns>
        public static bool AddWatermarkAndSave(string path, string fileName, string text, Image img,
                    int paddingTop, int paddingLeft, Color textColor,bool isWaterMark,out string imgurl)
        {
            string basePath = $"g:/iisIPiePush/Image/{path}/";
            string imgformat = "";
            if (!Directory.Exists(basePath))
            {
                Directory.CreateDirectory(basePath);
            }
            Font textFont = new Font("华文彩云", 45, FontStyle.Bold);
            ...
            var imgtype = GetImageFormat(img, out imgformat);
            bm.Save($"{basePath}{fileName}{imgformat}", imgtype);
            bm.Dispose();
            imgurl = $"http://image.ipipe.com/{path}{fileName}{imgformat}";
            return true;
        }
        #endregion
        /// <summary>
